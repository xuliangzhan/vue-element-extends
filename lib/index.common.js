...
// CONCATENATED MODULE: ./packages/editable/src/editable.vue?vue&type=template&id=56acc5e2&
  ...
  methods: {
    ...
    _initial: function _initial(datas, isReload) {
      ...
      this.$nextTick(function () {
        if (_this11.highlightCurrentRow) {
          var matchObj = _this11.currentRow ? external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.findTree(_this11.datas, function (row) {
            return row.data === _this11.currentRow;
          }, _this11.elTreeOpts) : null;

          _this11.$refs.refElTable.setCurrentRow(matchObj ? matchObj.item : null);
        }
      });
    },
    ...
    _toDatas: function _toDatas(datas, status) {
      ...
      return external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.mapTree(datas, function (item) {
        return _this12._toData(item, status);
      }, {
    ...
    _setDefaultChecked: function _setDefaultChecked() {
      ...
      this.$nextTick(function () {
        external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.eachTree(_this14.datas, function (row) {
          if (row.data._checked) {
            _this14.$refs.refElTable.toggleRowSelection(row, true);
          }
        }, _this14.elTreeOpts);
      });
    },
                    ...
                    if (offsetRow) {
                      ...
                      if (this.configs.mode === 'cell' && row.editActive) {
                        ...
                        this._validCellRules('blur', row, column).then(function () {
                          ...
                          return _this15._toValidError(rule, row, column, _cell);
                        });
                      } else {
                        row.checked = null;
                        offsetRow.checked = _offsetColumn.property;
        ...
        if (_row) {
          ...
          if (_column) {
            ...
            this._triggerActive(_row, _column, _cell2, event).then(function () {
              if (_this15.configs.checkedEditMethod ? _this15.configs.checkedEditMethod({
                row: _row.data,
                rowIndex: _rowIndex,
                column: _column,
    ...
    _triggerClear: function _triggerClear(evnt) {
      ...
      if (this.configs.autoClearActive && this.lastActive && (this.callEvent ? this.callEvent.vT < now_default()() : true)) {
        var _ret = function () {
          ...
          var clearActiveMethod = _this16.configs.clearActiveMethod;
          var _this16$lastActive = _this16.lastActive,
              row = _this16$lastActive.row,
              column = _this16$lastActive.column,
              cell = _this16$lastActive.cell;

          var rowIndex = _this16._getRowIndex(row);
          ...
          while (target && target.nodeType && target !== document) {
            if (_this16.configs.mode === 'row') {
              if (target === trElem) {
                return {
                  v: void 0
                };
              } else if (util.hasClass(target, 'editable-row')) {
              ...
              } else if (util.hasClass(target, 'editable-column')) {
                type = 'in';
              }
            }

            if (type && util.hasClass(target, 'editable')) {
              if (target !== _this16.$el) {
                type = 'out';
              }
          ...
          if (clearActiveMethod) {
            ...
            if (_this16.configs.mode === 'cell') {
              assign_default()(param, {
                column: column,
                columnIndex: _this16._getColumnIndex(column)
              });
            }
          ...
          if (isClearActive) {
            _this16._validActiveCell().then(function () {
              ...
              if (_this16.configs.mode === 'row') {
                _this16.$emit('clear-active', row.data, evnt);

                _this16.$emit('blur-active', row.data, evnt);
              } else {
                _this16.$emit('clear-active', row.data, column, cell, evnt);

                _this16.$emit('blur-active', row.data, column, cell, evnt);
              }
            }).catch(function (e) {
              return e;
            });
    ...
    _cellHandleEvent: function _cellHandleEvent(type, row, column, cell, event) {
      ...
      // 触发顺序 -> clear -> active
      setTimeout(function () {
        if (_this17.configs.trigger !== 'manual' && util.hasClass(cell, 'editable-col_edit') && (row.editActive ? _this17.configs.mode === 'row' && _this17.lastActive ? _this17.lastActive.column.property !== column.property : _this17.configs.mode === 'cell' && row.editActive !== column.property : true)) {
          _this17._validActiveCell().then(function () {
            if (_this17.lastActive) {
              _this17._clearValidError(_this17.lastActive.row);
            }

            if (_this17.configs.trigger === type) {
              _this17._triggerActive(row, column, cell, event);

              if (_this17.configs.mode === 'row') {
                _this17._validRowRules('change', row).catch(function (_ref11) {
                ...
                _this17._validCellRules('change', row, column).catch(function (rule) {
                  return _this17._toValidError(rule, row, column, cell);
                });
              }
            } else {
              _this17.datas.forEach(function (item) {
            ...
            return _this17.$emit("cell-".concat(type), row.data, column, cell, event);
          });
        } else {
          _this17.$emit("cell-".concat(type), row.data, column, cell, event);
        }
      });
    },
    _triggerClearChecked: function _triggerClearChecked(evnt) {
      ...
      while (target && target.nodeType && target !== document) {
        if (!isRow && util.hasClass(target, 'editable-row')) {
          isRow = true;
        }

        if (!isColumn && util.hasClass(target, 'editable-column')) {
          isColumn = true;
        }
    ...
    _clearActiveData: function _clearActiveData() {
      ...
      this._getTDatas().forEach(function (row) {
        row.editActive = null;
        row.showValidMsg = false;
      });
    },
    ...
    _triggerActive: function _triggerActive(row, column, cell, evnt) {
      ...
      return new promise_default.a(function (resolve, reject) {
        if (!_this18._isDisabledEdit(row, column)) {
          _this18._restoreTooltip(cell);

          _this18._disabledTooltip(cell);

          _this18._clearChecked();
          ...
          _this18.$nextTick(function () {
            _this18._scrollIntoView(cell);

            _this18._setCellFocus(cell);

            if (_this18.configs.mode === 'row') {
              _this18.$emit('edit-active', row.data, evnt);
            } else {
              _this18.$emit('edit-active', row.data, column, cell, evnt);
            }
          ...
          if (_this18.configs.mode === 'row') {
            _this18.$emit('edit-disabled', row.data, evnt);
          } else {
            _this18.$emit('edit-disabled', row.data, column, cell, evnt);
          }
            ...
            validPromise = validPromise.then(function (rest) {
              return new promise_default.a(function (resolve, reject) {
                _this19._validCellRules('all', row, column).then(resolve).catch(function (rule) {
                  var _this19$_getColumnByR = _this19._getColumnByRowIndex(null, column.property, row.data),
    ...
    _getColumnByRowIndex: function _getColumnByRowIndex(rowIndex, property, record) {
      var row;
      ...
      if (rowIndex === null) {
        rowIndex = external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.findIndexOf(tableData, function (row) {
          return row.data === record;
        });
      }
    ...
    _toActiveRow: function _toActiveRow(record, prop) {
      ...
      var _this$_getColumnByRow4 = this._getColumnByRowIndex(null, prop, record),
    ...
    _toValidError: function _toValidError(rule, row, column, cell) {
      ...
      this._triggerActive(row, column, cell, {
        ...
        if (!_this22.configs.useDefaultValidTip) {
        ...
        _this22.$emit('valid-error', rule, row, column, cell);
      });
    },
    ...
    _getCsvLabelData: function _getCsvLabelData(opts, columns) {
      ...
      return this._getTDatas().map(function (row, rowIndex) {
        ...
        columns.forEach(function (column) {
          var cell = trElem.querySelector(".".concat(column.id));
          item[column.id] = cell ? cell.innerText.trim() : external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.get(row.data, column.property);
        });
        return item;
      });
    },
    _getCsvContent: function _getCsvContent(opts) {
      ...
      var datas = opts.data ? opts.data : isOriginal ? this._getData() : this._getCsvLabelData(opts, columns);
    ...
    refresh: function refresh(force) {
      ...
      return this.$nextTick().then(function () {
        if (!force && _this23.lastOperation) {
          var expandeKeys = _this23.lastOperation.expandeKeys;
          expandeKeys.forEach(function (key) {
            return _this23.$refs.refElTable.store.toggleTreeExpansion(key);
          });
        ...
        records.forEach(function (record) {
          ...
          if (matchObj) {
            ...
            if (removeRow.editStatus === 'initial') {
              _this24.deleteRecords.push(removeRow);
            }
    ...
    getUpdateRecords: function getUpdateRecords() {
      ...
      return this._getData(external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.filterTree(this.datas, function (item) {
        return item.editStatus === 'initial' && !external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.isEqualWith(item.data, item.store, function (v1, v2, key) {
          if (key === _this25.elTreeOpts.children) {
            return true;
          }
        });
      }), this.elTreeOpts);
        ...
        if (scope) {
          ...
          if (cell) {
            return _this26._validCellRules(row.validActive ? 'all' : 'change', row, column).then(function (rule) {
              ...
              return _this26._toValidError(rule, row, column, cell);
            }).then(function () {
