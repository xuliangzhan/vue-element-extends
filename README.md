...
| clearActiveMethod | 该函数 Function((row[, column]?, index) 的返回值用来决定是否允许清除活动编辑 | Function | — | — |
...
3. 将 Editable.vue 和 EditableColumn.vue 复制到项目中，（例：components 目录下），然后注册组件

😱可编辑表格对组件中依赖双向绑定属性较多，渲染性能开销大，建议数据不要超过 200 行，使用分页加载😱  
