...
var ScrollHandle = {
  ...
  scroll: function scroll() {
    return function (evnt) {
      ...
      if (isRender) {
        // 超过阈值重新渲染
        var toVisibleStart = isTop ? toVisibleIndex - 1 : toVisibleIndex - Math.floor(visibleSize / 2);
        ...
        if (toVisibleStart !== visibleStart) {
          console.log("isTop=".concat(isTop, " toVisibleStart=").concat(toVisibleStart));
  ...
  compute: function compute(size) {
    ...
    return function (isReload) {
      if (this.scrollLoad) {
        ...
        var renderSize = getRenderSize(this.configs, this.elemStore, visibleSize, defSize);
...
var editable_component = normalizeComponent(
  ...
  editablevue_type_template_id_0dcdc942_render,
  editablevue_type_template_id_0dcdc942_staticRenderFns,
  false,
  null,
  null,
  null
